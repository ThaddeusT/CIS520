...
//change here
static bool  thread_low_priority (const struct list_elem *lhs, const struct list_elem *rhs, void *aux UNUSED);
               ...
               thread_func *function, void *aux) 
{
  ...
  if (priority > thread_current ()->priority)
    thread_yield_higher_prioprity (thread_current ()); 
...
thread_unblock (struct thread *t) 
{
  ...
  //change here
  list_insert_ordered (&ready_list, &t->elem,  thread_low_priority, NULL);
...
thread_yield (void) 
{
  ...
  if (cur != idle_thread)
    ...
    //change here
    list_insert_ordered (&ready_list, &cur->elem,  thread_low_priority, NULL);
...
//change here
void
thread_yield_higher_prioprity (struct thread *cur)
{
  enum intr_level old_level;
...
thread_set_nice (int nice /* Old Implementation UNUSED */) 
{
  ...
  curr->nice = nice;
  thread_calculate_recent_cpu_other (curr);
  thread_calculate_priority_other  (curr);
}
 ...
 */
static bool
 thread_low_priority (const struct list_elem *lhs, const struct list_elem *rhs, void *aux UNUSED)
{
...
sort_thread_list (struct list *l)
{
  ...
  list_sort (l,  thread_low_priority, NULL);
}

/* same as  thread_low_priority but use > instead of >= */
static bool
